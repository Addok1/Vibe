...
class SetPriceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $zoneType = $zone->zoneType()->create([
            ...
            'app_module_id' => $request->app_module_id,
            ...
            'enable_goods_type' => $request->enable_goods_type,
...
// dd($zoneType);
    ...
    // dd($request->all());
        ...
        $zoneType = $zoneTypePrice->zoneType;
        $targetZoneId = $request->zone_id;
        $targetVehicleTypeId = $request->vehicle_type;
        $targetTransportType = $transportType;

        $conflict = ZoneType::query()
            ->where('zone_id', $targetZoneId)
            ->where('type_id', $targetVehicleTypeId)
            ->where('transport_type', $targetTransportType)
            ->where('id', '!=', $zoneType->id)
            ->exists();
        ...
        if ($conflict) {
            return response()->json([
                ...
                'errors' => [
                    'zone_id' => ['This zone already has set price for the selected vehicle type.'],
                ],
            ], 422);
        }
