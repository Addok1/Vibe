...
class PaymentController extends BaseController
{
    ...
    {
        $query = CardInfo::where('user_id', auth()->user()->id);

        if ($this->gateway === 'stripe') {
            $stripeEnv = get_payment_settings(Settings::STRIPE_ENVIRONMENT) == 'test' ? 'test' : 'live';
            $query->where('merchant_id', 'from-stripe');

            if ($stripeEnv === 'test') {
                $query->where(function ($q) {
                    $q->where('stripe_environment', 'test')->orWhereNull('stripe_environment');
                });
            } else {
                $query->where('stripe_environment', 'live');
            }
        }
    ...
    public function makeDefaultCard(Request $request)
    {
        ...
        $card_info = (clone $query)->where('is_default', true)->first();
        ...
        (clone $query)->where('id', $request->card_id)->update(['is_default' => true]);
    ...
    public function walletHistory()
    {
        ...
        $settings = [
            ...
            'enable_pawapay' => get_payment_settings('enable_pawapay'),
        ];
        ...
        $images = [
            ...
            'pawapay' => asset('assets/img/pawapay.png'),
        ];
    ...
    public function requestForWithdrawal(Request $request)
    {
        ...
        if (access()->hasRole(Role::USER)) {
            ...
            $minimum_wallet_withdrawal_amount = get_settings(Settings::MINIMUM_WALLET_AMOUNT_FOR_WITHDRAWAL);

            if ($wallet_balance < $minimum_wallet_withdrawal_amount) {

                //Throw exception
                
               $this->throwCustomException(
                    'Minimum Wallet amount is ' . $minimum_wallet_withdrawal_amount
                );
