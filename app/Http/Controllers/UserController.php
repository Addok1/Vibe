...
class UserController extends BaseController
{
    ...
    public function index() 
    {
        ...
        return Inertia::render('pages/user/index',[
            'app_for'=>$app_for,
            'enable_user_document_upload' => $enableUserDocUpload
        ]);
    }
    // List of User
    public function list(QueryFilterContract $queryFilter, Request $request)
    {
        ...
        if ($request->has('approvestatus')) {
            $query->where('approve', (int) $request->approvestatus);
        }
...
// dd($results);
    ...
    public function store(Request $request)
    {
        ...
        // Set approve status
        $created_params['approve'] = $enableUserDocUpload == 1 ? 0 : 1;
        // Create a new User
...
// dd($user);
        ...
        return Inertia::render(
            'pages/user/create', ['countries'=>$result['data'], 'enable_user_document_upload' => $enableUserDocUpload,
            'default_dial_code'=>$default_dial_code,'user'=>$user,'app_for'=>$app_for,'default_flag'=>$default_flag,'default_country_id'=>$default_country_id]
        );
    }
    ...
    // dd($request->all());
        ...
        // Validate the incoming request
            ...
            // Set approve status
            if ($enableUserDocUpload == 1) {
                $hasDocument = $user->userDocumentDetail()->exists();

                $updated_params['approve'] = $hasDocument ? 1 : 0;
            } else {
                $updated_params['approve'] = 1;
            }
...
// dd($updated_params);
        ...
        {
            $user = User::find($request->id);
            $user->update(['approve'=> $request->status]);

            return response()->json([
                'successMessage' => 'User approval status updated successfully',
            ]);
        }
...
// dd($updated_params);
    ...
    public function UpdateUserDeclineReason(Request $request)
    {
        ...
        $notification = DB::table('notification_channels')
            ->where('topics', 'User Account Disapproval')
            ->first();
        ...
        if ($notification && $notification->mail == 1) {
            if (!empty($user->email)) {
                SendUserAccountDisapprovedMailNotification::dispatch($user);
            }
        }
