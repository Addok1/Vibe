...
class LoginController extends ApiController
{
    ...
    {
        // dd($request->all());
        $agent = Agents::where('email', $request->email)
            ->select('active')
            ->first();
        ...
        if ($agent->active != 1) {
            return response()->json([
                'status' => 'failed',
                'errors' => ['email' => ['Agent is not approved by Admin.']]
            ], 422);
        }
    ...
    {
        $owner = Franchise::where('email', $request->email)
            ->select('approve')
            ->first();
    ...
    protected function loginUserWithMobile($request, $role, $needsToken = true, array $conditions = [])
    {
        ...
        if (!$user) {
            ...
            $this->throwAccountMobileNotFoundException();
        }
    ...
    protected function loginUserWithPassword($request, $role, $needsToken = true, array $conditions = [])
    {
        ...
        if (!hash_check($password, $user->password)) {
            ...
            $this->throwAccountPasswordWrongException();
        }
    ...
    public function dynamicLoginUrl($redirect) 
    {
        ...
        $enableGoogleSocial = (string) ($social['enable_google_social_login'] ?? '0') === '1';
        $enableFacebookSocial = (string) ($social['enable_facebook_social_login'] ?? '0') === '1';
        $enableAppleSocial = (string) ($social['enable_apple_social_login'] ?? '0') === '1';
        ...
        return Inertia::render('pages/landing/user-web/index',[
            ...
            'enable_google_social_login' => $enableGoogleSocial,
            'enable_facebook_social_login' => $enableFacebookSocial,
            'enable_apple_social_login' => $enableAppleSocial,
        ]);
