...
class UserFilter implements FilterContract {
	...
	public function filters() {
		return [
			'status','date_option','search','country','unit','timezone',  'approvestatus','social_provider'
		];
	}
    /**
    ...
    */
    ...
    public function approvestatus($builder, $value = null) 
    {
        if ($value !== null) {
            $builder->where('approve', (int) $value);
        }
    }
   ...
   public function date_option($builder, $value = 0)
    ...
    public function social_provider($builder, $value = null) 
    {
        if ($value === 'all') {
        $builder->whereIn('social_provider', ['facebook', 'google', 'apple']);
        } else {
