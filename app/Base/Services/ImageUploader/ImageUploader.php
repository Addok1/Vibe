...
class ImageUploader implements ImageUploaderContract
{
    ...
    public function saveBannerPreview()
    {
        $this->validateFile();
    
        $config = $this->config('banner.upload.images');
    ...
    public function saveUserDocument($user_id)
    {
        $this->validateFile();
    
        $config = $this->config('user.upload.documents');
    
        $image = $this->file;
        $file_format = $image->getClientOriginalExtension();
        $filename = $this->hashGenerator->extension($file_format)->make();
    
        // Create a file path that includes the user ID
        $filePath = file_path(data_get($config, 'path'),$user_id);
