...
use App\Mail\UserDisapproveMail;
use App\Models\Admin\NotificationChannel;

class SendUserAccountDisapprovedMailNotification implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $notification = NotificationChannel::where('topics', 'User Account Disapproval') // Match the correct topic
        ->first();
        ...
        // if(!empty($user?->email)){
            if ($notification && $notification->mail == 1) {
              ...
              'footer_copyrights' => $translation->footer_copyrights ?? $notification->footer_copyrights,
               ...
               // Send welcome email
               Mail::to($this->user->email)->send(new UserDisapproveMail($this->user, $notificationData));
           }
        // }
