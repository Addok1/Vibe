...
if (!function_exists('find_peak_zone')) {
    function find_peak_zone($lat, $lng, $time = null)
    {
        ...
        $query = PeakZone::wherecontains('coordinates', $point)->where('active', 1);
...
if (!function_exists('custom_invoice_trans')) {
    function custom_invoice_trans($key, $replace = [], $locale = null)
    {
        $locale = $locale ?: app()->getLocale();
        $filePath = public_path("lang/{$locale}/invoice_translation.json");
...
if (!function_exists('get_user_locations')) {
    function get_user_locations($user)
    {
        ...
        elseif (get_settings('dispatcher-addons') == 1 && !empty($user->agent) &&$user->agent()->exists()) {
            $service_location = ServiceLocation::where('active', true)->where('id', $user->agent->service_location_id)->get();
        }
        elseif (get_settings('franchise-addons') == 1 && !empty($user->franchise) && $user->franchise()->exists()) {
            $service_location = ServiceLocation::where('active', true)->where('id', $user->franchise->zoneDetail->serviceLocation->id)->get();
        }
        else {
            $service_location = ServiceLocation::where('active', true)->where('id', $user->admin->service_location_id)->get();
        }
...
if (!function_exists('get_user_location_ids')) {
    function get_user_location_ids($user)
    {
        ...
        elseif (get_settings('franchise-addons') == 1 && !empty($user->franchise) && $user->franchise()->exists()) {
            $service_location = ServiceLocation::where('active', true)->where('id', $user->franchise->zoneDetail->serviceLocation->id)->pluck('id');
        }
        else {
            $service_location = [];
        }
