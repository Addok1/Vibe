...
use App\Transformers\Common\AppModuleBannerImageTransformer;
...
class UserTransformer extends Transformer
{
    ...
    protected array $defaultIncludes = [
        'sos','bannerImage','wallet','appModuleBannerImage'
...
// dd(get_settings(Settings::SHOW_RIDE_WITHOUT_DESTINATION));
        ...
        $params['show_wallet_add_money_feature_on_mobile_app'] = get_settings('shoW_wallet_add_money_feature_on_mobile_app');
        ...
        $params['enable_user_document_upload'] = get_settings('enable_user_document_upload');
        ...
        $params['enable_invoice_download'] = get_settings('enable_invoice_download'); 
       ...
       $params['enable_luggage_preference_for_user'] = (get_settings(Settings::ENABLE_LUGGAGE_PREFERENCE_FOR_USER));
        ...
        $params['enable_map_show_home_screen'] = get_settings(Settings::ENABLE_MAP_SHOW_HOME_SCREEN);
        $params['map_show'] = get_map_settings('map_show');

        $params['osm_map_settings'] = [
            'enable_mapbox' => filter_var(get_map_settings('enable_mapbox') ?? false, FILTER_VALIDATE_BOOLEAN),
            'mapbox_public_key' => get_map_settings('mapbox_public_key'),
            'enable_thunderforest' => filter_var(get_map_settings('enable_thunderforest') ?? false, FILTER_VALIDATE_BOOLEAN),
            'thunderforest_api_key' => get_map_settings('thunderforest_api_key'),
            'enable_stadia' => filter_var(get_map_settings('enable_stadia') ?? false, FILTER_VALIDATE_BOOLEAN),
            'stadia_api_key' => get_map_settings('stadia_api_key'),
        ];
    ...
    */
    ...
    public function includeAppModuleBannerImage()
    {
        $bannerImageModel = BannerImageModel::with('appModule')->where('active', true)->get();

        return $bannerImageModel
        ? $this->collection($bannerImageModel, new AppModuleBannerImageTransformer)
        : $this->null();
    }
