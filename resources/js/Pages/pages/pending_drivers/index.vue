...
export default {
            ...
            try {
                ...
                if (response.data.data === 'uploaddocument') {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.open(`/approved-drivers/document/${driver.id}?return_to=pending&page=${paginator.value.current_page || 1}`, '_blank');
                        }
                    });
                } else if (response.data.status === 'success') {
                    Swal.fire(t('success'), t('driver_approved_successfully') || 'Driver approved successfully.', 'success');
                    fetchDatas();
                }
            } catch (error) {
                console.error(error);
                Swal.fire(t('error'), t('failed_to_approve_driver_document') || 'Failed to approve driver.', 'error');
            }
        };
    ...
    mounted() {
        ...
        const initialPage = pageFromUrl ? Number(pageFromUrl) : (savedPage ? Number(savedPage) : 1);
        this.handlePageChanged(initialPage);
    },
};
</script>
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td v-if="permissions.includes('driver-upload-documents')">
                                            ...
                                            <a :href="`/approved-drivers/document/${result.id}?return_to=pending&page=${paginator.current_page || 1}`" target="_blank">
                                                <i class="bx bxs-file text-primary" style="font-size: 35px"></i>
                                            </a>
                                        </td>
        ...
        <BOffcanvas v-model="rightOffcanvas" placement="end" :title="$t('filters')" header-class="bg-light"
            ...
            <BFrom action="" class="d-flex flex-column justify-content-end h-100">
                <div class="offcanvas-body">
                    ...
                    <div class="mb-4">
                        <label for="expired-document-filter"
                            class="form-label text-muted text-uppercase fw-semibold mb-3">{{$t("expired_document_drivers")}}</label>
                        <select class="form-control" v-model="filter.has_expired_document" id="expired-document-filter">
                            <option :value="null">{{$t("all")}}</option>
                            <option value="1">{{$t("only_expired_document_drivers")}}</option>
                        </select>
                    </div>
                    <div class="mb-4">
                        <label for="created-within-days"
                            class="form-label text-muted text-uppercase fw-semibold mb-3">{{$t("newly_created_accounts")}}</label>
                        <select class="form-control" v-model="filter.created_within_days" id="created-within-days">
                            <option :value="null">{{$t("all")}}</option>
