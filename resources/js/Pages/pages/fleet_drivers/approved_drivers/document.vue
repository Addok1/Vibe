...
function getReturnToPendingParams() {
    const params = new URLSearchParams(typeof window !== 'undefined' ? window.location.search : '');
    return {
        returnTo: params.get('return_to') === 'pending',
...
export default {
    ...
    setup(props) {
        ...
        const errors = ref({});
        const returnToPending = ref(false);
        const returnPage = ref('1');

        onMounted(() => {
            const { returnTo, page } = getReturnToPendingParams();
            returnToPending.value = returnTo;
        ...
        const getRedirectAfterApproval = () => {
            if (returnToPending.value) {
                return `/fleet-drivers/pending?page=${returnPage.value}`;
            }
        ...
        const approveDocument = async (documentId) => {
            ...
            if(response.data.allDocumentsApproved == true){
                ...
                router.get(getRedirectAfterApproval());
            }
            else{
                const query = returnToPending.value ? `?return_to=pending&page=${returnPage.value}` : '';
                router.get(`/fleet-drivers/document/${props.driverId}${query}`);
            }
        ...
        const declineDocument = async (documentId) => {
           ...
           const handleSubmit = async () => {
            ...
            if(response.data.allDocumentsDisapproved == true){
                ...
                router.get(returnToPending.value ? `/fleet-drivers/pending?page=${returnPage.value}` : '/fleet-drivers/pending');
            }
            else{
                router.get(`/fleet-drivers/document/${props.driverId}${returnToPending.value ? `?return_to=pending&page=${returnPage.value}` : ''}`);
            }    
        } catch(error){
