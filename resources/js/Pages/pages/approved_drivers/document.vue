...
export default {
    ...
    setup(props) {
        ...
        const getRedirectAfterApproval = () => {
            if (returnToPending.value) {
                return `/pending-drivers?page=${returnPage.value}`;
            }
        ...
        const approveDocument = async (documentId) => {
            const status = 1;
                ...
                if(response.data.allDocumentsApproved == true){
                    ...
                    router.get(`/approved-drivers/document/${props.driverId}${query}`);
                }
        };
    ...
    try {
        ...
        if(response.data.allDocumentsDisapproved == true) {
            ...
            router.get(returnToPending.value ? `/pending-drivers?page=${returnPage.value}` : '/pending-drivers');
        } else {
            router.get(`/approved-drivers/document/${props.driverId}${returnToPending.value ? `?return_to=pending&page=${returnPage.value}` : ''}`);
        }
