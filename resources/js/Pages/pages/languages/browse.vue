    ...
    setup(props) {
        ...
        const downloadFile = async () => {
            try {
                ...
                let fileName = '';
                if(form.group === 'user_app' || form.group === 'driver_app'){
                    fileName = `${form.group}.arb`;
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', fileName);// Modify the file name as needed
                    document.body.appendChild(link);
                    link.click();
                    document.body.removeChild(link);
                }
                else{
                    fileName = `${form.group}.json`;
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', fileName); // Modify the file name as needed
                    document.body.appendChild(link);
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="3">
                                ...
                                <select id="select_languages" class="form-select" v-model="form.group" @change="handleGroupChange">
                                    ...
                                    <option value="invoice_translation">{{$t("invoice_translation")}}</option>
                                </select>
                            </BCol>
