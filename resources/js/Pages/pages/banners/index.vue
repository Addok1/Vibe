...
export default {
    ...
    setup(props) {
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/bannerimage/delete/${dataId}`);
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/bannerimage/list`, { params });
        ...
        const toggleActiveStatus = async (id, status) => {
            try {
                await axios.post(`/bannerimage/update-status`, { id: id, status: status });
        ...
        const handlePageChanged = async (page) => {
            localStorage.setItem("bannerimage/list", page); // Save to localStorage
            fetchDatas(page);
        };

        const editData = async (result) =>  {
            router.get(`/bannerimage/edit/${result.id}`); 
        };
    ...
    mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("bannerimage/list");
        if(savedPage){
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">                                   
                                    <Link href="/bannerimage/create" v-if="permissions.includes('add_banner_image')">
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td>{{ result.title}}</td>
                                        <td>
                                            <span v-if="result.bannertype == 1">
                                                {{ result.appmodule?.name }}
                                            </span>
                                            <span v-else>-</span>
                                        </td>
                                        <td>
