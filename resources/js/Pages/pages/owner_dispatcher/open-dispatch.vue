...
export default {
  ...
  return formattedTime;
    ...
    const drawRoute = () => {
      ...
      console.log("alwaypoints:",allWayPoints);
  ...
  errors.value.map = t('service_not_available');
        ...
        control = L.Routing.control({
          ...
          console.log('poly_line',form.poly_line);
...
watch(() => etaParam.duration, (newDuration) => {
  ...
  try {
      const response = await axios.post("/owner/serviceVerify", payload );
...
if(stopovers.length > 0) {
  ...
  rentalVehicleTypes.value = [];
      ...
      if(form.promo_code){
       ...
       try{
                const response = await axios.post(`/owner/request/eta`, payload);
...
try {
    ...
    onMounted(async () => {
      fetchPromoList();
        ...
        console.log("appModules:", appModules.value);
        console.log("driverList:", driverList.value);
        console.log("driver.ownerid:", driver.ownerid);
        console.log("props.owner.id:", props.owner?.id);
...
const  getMinReturnTime = () =>{
 ...
 const fetchPromoList = async (page = 1) => {
    try {
      const response = await axios.get(`/owner/promocode-list`);
      console.log("Promo API:", response.data);
      results.value = response.data.data;
      console.log("results.value",results.value);
      modalFilter.value = false;
    } catch (error) {
      console.error(t('error_fetching_promo'), error);
    }
  };
