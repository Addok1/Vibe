...
export default {
  ...
  setup(props) {
    ...
    const handleSubmit = async () => {
      ...
      if (form.mobile && (!props.admin || form.mobile !== props.admin.mobile)) {
        const mobileCheckResponse = await axios.get(`/admins/check-mobile/${form.mobile}${props.admin ? `/${props.admin.id}` : ''}`);
      ...
      if (form.email && (!props.admin || form.email !== props.admin.email)) {
        const emailCheckResponse = await axios.get(`/admins/check-email/${form.email}${props.admin ? `/${props.admin.id}` : ''}`);
