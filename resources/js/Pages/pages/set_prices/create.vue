...
export default {
  ...
  setup(props) {
    ...
    const form = useForm({
      ...
      // app_module_id: props.zoneType ? props.zoneType.app_module_id || "" : "",
     ...
     admin_commision: props.zoneType ? props.zoneType.admin_commision ?? 0 : "",
      ...
      enable_goods_type: props.zoneType ? props.zoneType.enable_goods_type ||  0 : 0,
    ...
    const fetchAppModules = async (transportType = '') => {
      try {
        let list = props.app_modules || [];
        if (!list.length) {
          const response = await axios.get('/app_modules/list', { params: { per_page: 1000 } });
          const raw = response?.data?.results ?? response?.data?.data ?? [];
          list = Array.isArray(raw) ? raw : (raw?.data ?? []);
        }
        const activeList = list.filter((mod) => String(mod.active) === '1' || mod.active === true);
        if (!transportType || transportType === 'both') {
          appModules.value = activeList;
        } else {
          const filtered = activeList.filter((mod) => !mod.transport_type || mod.transport_type === 'both' || mod.transport_type === transportType);
          appModules.value = filtered.length ? filtered : activeList;
        }
        console.log('[set_prices] transport_type:', transportType, 'modules:', appModules.value);
      } catch (error) {
...
const fetchVehicleTypes = async () => {
  ...
  watch(() => form.transport_type, (newTransportType, oldTransportType) => {
    ...
    fetchAppModules(newTransportType);
    if(newTransportType == 'delivery'){
      form.enable_shared_ride= false;

    }
    if (newTransportType !== 'delivery' && newTransportType !== 'both') {
      form.enable_goods_type = 0;
    }
  });

  watch(() => form.zone_id, (newZoneId, oldZoneId) => {
    if (!newZoneId) return;
    fetchVehicleTypes();
    selectedZoneUnit();
  });
  ...
  onMounted(() => {
    ...
    fetchAppModules(form.transport_type);
    selectedZoneUnit();
  });
        ...
        <BCard no-body id="tasksList">
          ...
          <BCardBody class="border border-dashed border-end-0 border-start-0">
            <form @submit.prevent="handleSubmit">
             ...
             <!--     <div class="col-sm-6">
                    <div class="mb-3">
                      <label for="app_module" class="form-label">{{$t("app_modules")}}
                        <span class="text-danger">*</span>
                      </label>
                      <select id="app_module" class="form-select" v-model="form.app_module_id">
                        <option disabled value="">{{$t('choose_app_services')}}</option>
                        <option v-for="module in appModules" :key="module.id" :value="module.id">{{ module.name }}</option>
                      </select>
                      <span v-for="(error, index) in errors.app_module_id" :key="index" class="text-danger">{{ error }}</span>
                    </div>
                  </div> -->
                  ...
                  <BCardFooter class="mt-5" v-if="form.transport_type === 'delivery' || form.transport_type === 'both'">
                    <h5>{{$t("goods_type")}}</h5>
                    <div class="row">
                      <div class="col-sm-6">
                        <div class="mt-3">
                          <div class="form-check form-check-inline">
                            <input
                              class="form-check-input"
                              type="checkbox"
                              id="enable_goods_type"
                              :checked="form.enable_goods_type"
                              @click="form.enable_goods_type = form.enable_goods_type ? 0 : 1"
                            />
                            <label class="form-check-label" for="enable_goods_type">{{$t("enable_goods_types")}}</label>
                          </div>
                        </div>
                      </div>
                    </div>
