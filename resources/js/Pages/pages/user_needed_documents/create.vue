    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (props.document && props.document.id) {
          response = await axios.post(`/user-needed-documents/update/${props.document.id}`, form.data());
        } else {
          response = await axios.post('store', form.data());
        }
        if (response.status === 201) {
          successMessage.value = t('user_needed_documents_created_successfully');
          form.reset();
          router.get('/user-needed-documents');
        } else {
          alertMessage.value = t('failed_to_create_user_needed_docs');
        }
      } catch (error) {
        if (error.response && error.response.status === 422) {
          errors.value = error.response.data.errors;
        } else {
          console.error(t('error_creating_user_needed_docs'), error);
          alertMessage.value = t('failed_to_create_user_needed_docs');
        }
      }
...
<template>
  <Layout>
    ...
    <PageHeader :title="document ? $t('edit') : $t('create')" :pageTitle="$t('user_needed_documents')" pageLink="/user-needed-documents"/>
    <BRow>
      <BCol lg="12">
