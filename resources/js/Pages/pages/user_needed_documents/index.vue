...
export default {
    ...
    setup(props) {
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/user-needed-documents/delete/${dataId}`);
        ...
        //     try {
        //         await axios.post(`/user-needed-documents/toggle`, { id: id, status: status });
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/user-needed-documents/list`, { params });
        ...
        const editData = async (result) =>  {
            router.get(`/user-needed-documents/edit/${result.id}`); 
        };
...
<template>
    <Layout>
        ...
        <Head title="user Needed Documents" />
        <PageHeader :title="$t('user_needed_documents')" :pageTitle="$t('user_needed_documents')" />
        <BRow>
            <BCol lg="12">
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <Link href="/user-needed-documents/create" v-if="permissions.includes('add-user-needed-document')">
                                    <BButton variant="primary" class="float-end"> <i
                                            class="ri-add-line align-bottom me-1"></i>  {{$t("add_user_needed_documents")}}</BButton>
                                    </Link>
                                </div>
                            </BCol>
                        </BRow>
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td v-if="permissions.includes('toggle-user-needed-document')">
                                            <div :class="{
                                                    'form-check': true,
                                                    'form-switch': true,
                                                    'form-switch-lg': true,
                                        ...
                                        <td>
                                            <BButton @click.prevent="editData(result)" v-if="permissions.includes('edit-user-needed-document')"
                                            ...
                                            <BButton class="btn btn-soft-danger btn-sm m-2" size="sm" v-if="permissions.includes('delete-user-needed-document')"
