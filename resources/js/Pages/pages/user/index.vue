...
export default {
    ...
    props: {
        ...
        alertMessage: String,
        app_for:String,
        enable_user_document_upload: Number,
    ...
    setup(props) {
        ...
        // const toggleApproveStatus = async (id, status) => {
        //     Swal.fire({
        //         title: t('are_you_sure'),
        //         text: t('you_are_about_to_change_status'),
        //         icon: "warning",
        ...
        //         if (result.isConfirmed) {
        //             try {
        //                 const response = await axios.post(`/user/update-approve`, { id: id, status: status });

        //                 if(response.data.data == "uploaddocument") {
        //                     Swal.fire(t('error'), t('failed_to_update_status'), "error");
        //                     router.get(`/user/document/${id}`);
        //                 }
        //                 else{
        //                     const index = results.value.findIndex(item => item.id === id);
        //                     if (index !== -1) {
        ...
        const viewDocument = async (user) =>  {
            window.open(`/user/document/${user.id}`, '_blank');
        };
        // const approveDocument = async (user) => {
        //     Swal.fire({
        //         icon: 'warning',
        //         title: t('documents_missing_or_need_review'),
        //         text: t('failed_to_upload_document'),
        //         showCancelButton: true,
        //         confirmButtonText: t('review_documents'),
    ...
    const approveDocument = async (user) => {
        // DISAPPROVE
        if (user.approve) {
            declinedModel(user.id);
            return;
        }
        try {

            const response = await axios.get(`/user/document-toggle/${user.id}`);
            ...
            Swal.fire(
                t('success'),
                t('user_approved_successfully') || 'User approved successfully.',
                'success'
            );
            ...
            Swal.fire(
                t('error'),
                t('failed_to_approve_user_document') || 'Failed to approve user.',
                'error'
            );
        }
    };
        const approveviewDocument = async (user) =>  {
            router.get(`/user/document/${user.id}`);
        };

        // const declinedModel = async (user) => {
        //     router.get(`/user/document/${user.id}`);
        // };
        ...
        const handleSubmit = async () => {
            try {
                const response = await axios.post(`/users/update/decline/reason`, {
                    'id': user_id.value,
                    'reason': form.reason
                });
                ...
                if (response.status === 200 || response.status === 201) {
                    ...
                    fetchDatas();
                    Swal.fire(t('success'), t('reason_declined'), 'success');
                } else {
        ...
        const fetchDatas = async (page = 1) => {
            ...
            params.approved = props.approved;
        }
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td v-if="permissions.includes('view-user-document')">
                                            <Link @click="viewDocument(result)" href="#"> 
                                                <i class="bx bxs-file text-primary" style="font-size: 35px"></i>
                                            </Link>
                                        </td>

                                        <td>
                                            <!-- <template v-if="enable_user_document_upload == 1">
                                            ...
                                            <template v-else>
                                              ...
                                              <template v-if="enable_user_document_upload == 0">
                                                <!-- Upload disabled → always approved -->
                                                <BBadge variant="success" class="text-uppercase">
                                                    {{$t("approved")}}
                                                </BBadge>
                                            </template>

                                            <template v-else>
                                                <!-- Upload enabled → check approval -->
                                                ...
                                                <template v-else>
                                                    <BBadge variant="danger" class="text-uppercase">
                                                        {{$t("disapproved")}}
                                                    </BBadge>
                                                </template>
                                            </template>
                                        </td>  
                                        <td>{{ result.reason ? result.reason : '--' }}</td> 
                                       ...
                                       <td v-if="permissions.includes('toggle-user')">
                                        ...
                                        <!-- <td v-if="permissions.includes('toggle-user')">
                                            <div :class="{
                                                ...
                                                <input class="form-check-input" type="checkbox" role="switch" @click.prevent="toggleApproveStatus(result.id,!result.approve)" :id="'status_'+result.id" :checked="result.approve">
                                            </div>
                                        </td> -->
                                       <td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item" href="#"  v-if="permissions.includes('user-approval')">
                                                        ...
                                                        <span v-else  @click.prevent="approveDocument(result)" >{{ $t('approve') }}</span>
                                                    </a>
                                    ...
                                    <div class="d-flex gap-1 align-items-center">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <a class="dropdown-item" href="#"  v-if="permissions.includes('user-approval')">
                                                    <i class="  bx bx-radio-circle-marked align-center text-muted me-2"></i> 
                                                    <span v-if="result.approve === 1" @click.prevent="viewDocument(result)">{{ $t('disapprove') }}</span>
                                                    <span v-else  @click.prevent="approveDocument(result)" >{{ $t('approve') }}</span>
                                                </a>
                                                <a class="dropdown-item" href="#" @click.prevent="viewProfile(result)" v-if="permissions.includes('view-user-profile')">
                                                    <i class="bx bx-radio-circle-marked align-center text-muted me-2"></i>{{$t("view_profile")}}
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="mb-3">
                            ...
                            <span v-if="errors.reason" class="text-danger">{{ errors.reason[0] }}</span>
                        </div>
                    </div>
                </div>
                        ...
                        <div class= "mt-5">
                            ...
                            <div>
                                <div class="form-check form-check-inline ">
                                    ...
                                    v-model.number="filter.approvestatus"
                                    type="radio"
                                    id="approve"
                                    :value="1"
                                    />

                                    <label class="form-check-label" for="approve">{{$t("approve")}}</label>
                                </div>
                                <div class="form-check form-check-inline">
                                <input 
                                    class="form-check-input"
                                    ...
                                    :value="0"
                                    />
                                    <label class="form-check-label" for="disapprove">{{$t("disapprove")}}</label>
                                </div>
                            </div>
                        </div>
                        <div class="mb-4 mt-5">
                            <label for="datepicker-range"
                                class="form-label text-muted text-uppercase fw-semibold mb-3">{{$t("social_provider")}}</label>
                            <select class="form-control" data-choices data-choices-search-false name="choices-select-status"
                                id="choices-select-status" v-model="filter.social_provider">
                                <option value="all">{{$t("all")}}</option>
