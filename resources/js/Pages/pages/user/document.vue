<template>
    <Layout>
        ...
        <PageHeader :title="$t('documents')" :pageTitle="$t('documents')" pageLink="/users"/>
                                ...
                                <tbody>
                                    <tr v-for="(document, index) in documents" :key="document.id">
                                        ...
                                        <td>
                                            ...
                                            title="upload" :href="`/user/document-upload/${document.id}/${userId}`">
...
export default {
    ...
    setup(props) {
        ...
        const approveDocument = async (documentId) => {
            const status = 1;
            const response = await axios.post(`/user/document-toggle/${documentId}/${props.userId}/${status}`);
            if(response.data.allDocumentsApproved == true){
                ...
                router.get(`/user/document/${props.userId}`); // Redirect or navigate to another route
            }
        ...
        const handleSubmit = async () => {
            try {
                const status = 5;
                const response = await axios.post(`/user/document-toggle/${document_id.value}/${props.userId}/${status}`,
                {
                    reason: form.reason
                }
                );
                ...
                router.get(`/user/document/${props.userId}`);
                }
        ...
        const updateDocuments = async () => {
            await axios.get(`/user/update/${props.userId}`);
            router.get(`/user`);
        };
