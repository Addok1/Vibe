...
export default {
  ...
  setup(props) {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (props.singlelandingHeader && props.singlelandingHeader.id) {
          response = await axios.post(`/single-landing-header-footer/update/${props.singlelandingHeader.id}`, formData, {
            headers: {
          ...
          response = await axios.post('/single-landing-header-footer/store', formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
        ...
        if (response.status === 201) {
          ...
          form.reset();
          router.get('/single-landing-header-footer');
        } else {
    ...
    const fetchStoredLanguages = async () => {     
      try {
        const response = await axios.get('/single-landing-header-footer/list');
