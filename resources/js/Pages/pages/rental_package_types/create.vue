...
export default {
  ...
  setup(props) {
    ...
    const ai_text = ref({})
    const generateAiText = async(request, field) =>{
      try {
      let payload = {};

      if (request == 'short') {
        if(form.short_description == ''){          
          alertMessage.value = t('Short description is required for generate');
          return;
        }
        else{          
          payload = {
         ...
         if(form.description == ''){          
          alertMessage.value = t('description is required for generate');
          return;
        }
        else{ 
          payload = {
      ...
      ai_text.value[field] = 'Genarating...'

      let response = await axios.post('/generate-ai-text', payload);
      console.log("response",response);

      if (request === 'short') {
          form.short_description = response.data.text;
      } else {
          form.description = response.data.text;
      }
            ...
            <form>
              <FormValidation :form="form" :rules="validationRules" ref="validationRef">
                <div class="row">
                    ...
                    <div class="mb-3">
                      <div class="d-flex justify-content-between">                        
                        <label for="short_description" class="form-label">{{$t("short_description")}}
                          ...
                          <button type="submit" class="btn" @click.prevent="generateAiText('short','short_description')" v-if="enableOpenAI == 1"> 
                            <img src="@assets/images/ai-icon.jpg" alt="ai-icon" width="23px">
                            <span class="text-primary fs-16">{{ai_text.short_description || 'Generate'}}</span>
                          </button>
                      </div>
                    ...
                    <div class="mb-3">
                      <div class="d-flex justify-content-between">                        
                        <label for="description" class="form-label">{{$t("description")}}
                          ...
                          <button type="submit" class="btn" @click.prevent="generateAiText('','description')" v-if="enableOpenAI == 1"> 
                            <img src="@assets/images/ai-icon.jpg" alt="ai-icon" width="23px">
                            <span class="text-primary fs-16">{{ai_text.description || 'Generate'}}</span>
                          </button>
                      </div>
                  ...
                  <div class="col-lg-12">
                    <div class="text-end">
                      <button type="submit" class="btn btn-primary" @click.prevent="handleSubmit"> {{ packageType ? $t('update') : $t('save') }}</button>
                    </div>
                  </div>
                </div>
              </FormValidation>
