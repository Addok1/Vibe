...
export default {
    ...
    setup(props) {
        ...
        onMounted(async () => {
            ...
            if (!document.querySelector(`script[src="https://maps.googleapis.com/maps/api/js?key=${googleMapKey}&libraries=places,drawing&v=3.64"]`)) {
                const script = document.createElement('script');
                script.src = `https://maps.googleapis.com/maps/api/js?key=${googleMapKey}&libraries=places,drawing&v=3.64`;
                script.onload = () => {
                    if (!mapInitialized) {
                        initializeMap();
                        // fetchServiceLocations();                        
                        console.log('Maps Version:', google.maps.version);
                        console.log('Drawing:', google.maps.drawing);
                        mapInitialized = true;
                    }
                };
