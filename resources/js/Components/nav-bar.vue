                                ...
                                <ul class="navbar-nav">
                                    <li class="nav-item dropdown" v-if="permissions.includes('access-home')">
                                        <Link class="nav-link nav-menu" href="/dashboard" id="dropdownMenuButton" role="button"  aria-expanded="false"
                                            ...
                                            '/push-notifications','/cancellation','/mail-template','/banner-image','/complaint-title','/report/user-report','/report/driver-report','/report/driver-duty-report',
                                            '/report/owner-report', '/report/finance-report','/report/fleet-report','/bannerimage',
                                    ...
                                    <li class="nav-item dropdown"  v-if="permissions.includes('access-user-nav-list')">
                                        <Link class="nav-link nav-menu" href="/users" id="dropdownMenuButton" role="button" aria-expanded="false"  
                                                :class="{ 'active': ['/manage-owners','/fleet-drivers/pending',
                                                'owner-needed-documents',  '/admins','/employee-franchise','/user/document',
                                                '/users', '/users/deleted-user',  '/negative-balance-drivers', '/withdrawal-request-drivers',
                                                '/delete-request-drivers', '/driver-needed-documents', 'profile-edit','/user-needed-documents',
                                                '/pending-drivers', '/approved-drivers','/owner-needed-documents','/driver-bank-info','/manage-payment',
                                                ...
                                                '/fleet-needed-documents','/driver-dashboard','/owner-dashboard','/withdrawal-request-owners','/category','/title','/support-tickets','/user-import','/driver-import','/owner/bookride','/approved-franchise-drivers','/pending-franchise-drivers','/manage-franchise-owner','/withdrawal-request-franchise','/franchise/promo-code','/franchise-owner-needed-documents','/franchiseowner-dashboard','/franchise/withdrawal-request','/report/franchiseowner-report'].some(path => $page.url.startsWith(path)) }"
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                           ...
                                           <li class="dropdown-submenu mb-2"  v-if="permissions.includes('owner-management')" >
                                            ...
                                            <li class="dropdown-submenu mb-2"  v-if="permissions.includes('drivers-management')" >
                                                ...
                                                '/delete-request-drivers', '/driver-needed-documents','/driver-dashboard',
                                    ...
                                    <li class="nav-item dropdown"  v-if="permissions.includes('access-settings-nav-list')">
                                        <Link class="nav-link nav-menu" href="/general-settings" id="dropdownMenuButton" aria-expanded="false" 
                                            ...
                                            '/wallet-settings','/payment-gateway','/sms-gateway','/tip-settings','/country','/map-show','/osm-map-setting',
                                            ...
                                            '/mail-configuration','/map-apis','/recaptcha','/notification-channel','/customization-settings','/landing-aboutus','/franchise-addons','/ai-setup','/social-auth-settings'].some(path => $page.url.startsWith(path)) }">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">                                                    
                                            ...
                                            <li class=" dropdown-submenu mb-2" v-if="permissions.includes('payment-gateway-settings-view')">
                                                ...
                                                '/firebase','/map-setting','/invoice-configuration','/map-show',
                                   ...
                                   <li class="nav-item dropdown"  v-if="permissions.includes('report-management')">
                                        <Link class="nav-link nav-menu" href="/report/user-report" id="dropdownMenuButton" role="button" aria-expanded="false"  
                                                :class="{ 'active': [ '/report/user-report','/report/driver-report','/report/owner-report'
                                               ].some(path => $page.url.startsWith(path)) }"
                                                @click="('/report/user-report')">
                                                    
                                            <i class=" ri-file-3-fill"></i>
                                            <span class="ms-2 dropdown-toggle">{{ $t("report") }}</span>
                                        </Link>
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li class="dropdown-submenu mb-2" v-if="permissions.includes('user-report')">
                                                <Link class="nav-link" href="/report/user-report"
                                                    :class="{ 'active': [ '/report/user-report',].some(path => $page.url.startsWith(path)) }">
                                                    {{ $t("user-report") }}
                                                </Link> 
                                            </li> 
                                           <li class="dropdown-submenu mb-2"  v-if="permissions.includes('driver-report')" >
                                                <Link class="nav-link" href="/report/driver-report"
                                                    :class="{ 'active': [ '/report/driver-report'].some(path => $page.url.startsWith(path)) }">
                                                    {{ $t("driver-report") }}
                                                </Link> 
                                            </li> 
                                            <li class="dropdown-submenu mb-2"  v-if="permissions.includes('owner-report')" >
                                                <Link class="nav-link" href="/report/owner-report"
                                                    :class="{ 'active': ['/report/owner-report'].some(path => $page.url.startsWith(path)) }">
                                                    {{ $t("owner-report") }}
                                                </Link> 
                                            </li> 
                                              <li class="dropdown-submenu mb-2"  v-if="permissions.includes('franchise-owner-report')" >
                                                <Link class="nav-link" href="/report/franchiseowner-report"
                                                    :class="{ 'active': ['/report/franchiseowner-report'].some(path => $page.url.startsWith(path)) }">
                ...
                >
                  ...
                  <i v-if="selectedLocation === location.value"
                    ...
                    <BDropdown class="dropdown" variant="ghost-secondary" dropstart
                        :offset="{ alignmentAxis: 55, crossAxis: 15, mainAxis: -50 }"
                        toggle-class="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle arrow-none"
                        menu-class="dropdown-menu-end"  boundary="viewport">
                   ...
                   <div class="ms-1 header-item d-none d-sm-flex">
                    ...
                    <div class="ms-1 header-item d-none d-sm-flex">
                      ...
                      <BDropdown variant="ghost-dark" dropstart class="ms-1 dropdown"
                        :offset="{ alignmentAxis: 57, crossAxis: 0, mainAxis: -42 }" boundary="viewport"
                        ...
                        <BTabs nav-class="dropdown-tabs nav-tab-custom bg-primary px-2 pt-2">
                          ...
                          <BTab title="Tickets" class="tab-pane fade py-2 ps-2" id="tickets-tab" role="tabpanel" aria-labelledby="tickets-tab">
                            <simplebar data-simplebar style="max-height: 300px" class="pe-2">
                              <div class="px-3 pt-2 text-muted" v-if="unreadTicket > 0">
                                {{ unreadTicket }} new ticket(s)
                              </div>
                              <div v-for="ticket in ticketNotifications" :key="ticket.id" class="text-reset notification-item d-block dropdown-item">
                                <div class="d-flex">
                                  <img src="/assets/images/Male_default_image.png" class="me-3 rounded-circle avatar-xs" alt="user-pic" />
                                  <div class="flex-grow-1">
                                    <BLink href="#" class="stretched-link" @click.prevent="handleNotificationClick(ticket.id)">
                                      <h6 class="mt-0 mb-1 fs-13 fw-semibold">
                                        {{ ticket.title }}
                                      </h6>
                                    </BLink>
                                    <p class="mb-0 fs-11 fw-medium text-uppercase text-muted">
                                      <span><i class="mdi mdi-clock-outline"></i> {{ formatTimestamp(ticket.updated_at) }}</span>
                                    </p>
                                  </div>
                                </div>
                              </div>

                              <div v-if="ticketNotifications.length === 0" class="p-3 text-center text-muted">
                                no new tickets
                              </div>
                            </simplebar>
                          </BTab>
                    ...
                    <BDropdown variant="link" class="ms-sm-3 header-item topbar-user"
                        ...
                        :offset="{ alignmentAxis: -14, crossAxis: 0, mainAxis: 0 }"  boundary="viewport">
                        <template #button-content>
