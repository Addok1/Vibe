...
// Listen for changes in notifications
      notificationsRef.on('value', (snapshot) => {
        ...
        if (data) {
          ...
          state.ticketNotifications = notificationsArray.filter((n) => {
            const title = (n.title || '').toLowerCase();
            const body = (n.body || '').toLowerCase();
            return title.includes('support ticket') || body.includes('support ticket');
          });
          state.unreadTicket = state.ticketNotifications.length;
